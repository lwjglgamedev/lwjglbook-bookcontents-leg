...
In order to detect a collision we must compare current position  value with the  value of the point of the terrain we are currently in. If we are above terrain’s  value there’s no collision, if not, we need to get back. Simple concept, does it ? Indeed it is but we need to perform several calculations before we are able to do that comparison.
...
![](.gitbook/assets/terrain_grid.png)
...
![Model to world coordinates](.gitbook/assets/model_to_world_coordinates.png)
...
Once we have found the terrain block, we need to calculate the triangle which we are in. This is done in the `getTriangle` method that will be described later on. After that, we have the coordinates of the triangle that we are in, including its height. But, we need the height of a point that is not located at any of those vertices but in a place in between. This is done in the  method. We will also explain how this is done later on.
...
*  is the  coordinate of the origin of the bounding box associated to the quad.
*  is the  coordinates  of the origin of the bounding box associated to the quad \(Although you see a “”, it models the  axis\).
...
![Terrain block variables](.gitbook/assets/terrain_block_variables_n.png)
...
Where m is the line slope, that is, how much the height changes when moving through the  axis. Note that, in our case, the  coordinates are the  ones. Also note, that we are using 2D coordinates because we are not calculating heights here. We just want to select the proper triangle and to do that  an  coordinates are enough. So, in our case the line equation should be rewritten like this.
...
So the equation of the diagonal to get the  value given a  position is like this:



Where  and  are the  and  coordinates of the vertices  and  respectively.
...
You can see that we have two additional methods. The first one, named `getDiagonalZCoord`, calculates the  coordinate of the diagonal given a  position and two vertices. The other one, named `getWorldHeight`, is used to retrieve the height of the triangle vertices, the  coordinate. When the terrain mesh is constructed the height of each vertex is pre-calculated and stored, we only need to translate it to world coordinates.
...
Where ,  and  are the three vertices needed to define the plane.

Then, with previous equations and the values of the  and  coordinates for the current position we are able to calculate the y value, that is the height of the terrain at the current position:
...
![Tunnelling](.gitbook/assets/tunnelling.png)

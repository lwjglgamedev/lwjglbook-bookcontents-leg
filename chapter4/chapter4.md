...
We use `MemoryUtil`class to create the buffer in off-heap memory so that it's accessible by the OpenGL library. After we have stored the data \(with the put method\) we need to reset the position of the buffer to the 0 position with the flip method \(that is, we say that we’ve finishing writing on it\).

If you come from previous versions of LWJGL it's important to stress out a few topics. You may have noticed that we do not use the utility class `BufferUtils`to create the buffers. Instead we use the `MemoryUtil`class. This is due to the fact that `BufferUtils`was not very efficient, and has been mantained obly for backwards compatibilty. Intead, LWJGL 3 proposes two methods for buffer management:
...
* Manually managed buffers. In this case we need to carefulley free them once we have finished. These buffers are intended for long time operations or for large amounts of data.

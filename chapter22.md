...
* A position, the location of the source \(,  and  coordinates\). By the way, OpenAL uses a right handed Cartesian coordinate system as OpenGL, so you can assume \(to simplify things\) that your world coordinates are equivalent to the ones in the sound space coordinate system.
...
![OpenAL concepts](.gitbook/assets/openal_concepts.png)
...
The other parameter, `relative`, controls if the position of the source is relative to the listener or not. In this case, when we set the position for a source, we basically are defining the distance \(with a vector\) to the listener, not the position in the OpenAL 3D scene, not the world position. This activated by the “`alSourcei(sourceId, AL_SOURCE_RELATIVE, AL_TRUE);”` call. But, What can we use this for? This property is interesting for instance for background sounds that should be affected \(attenuated\) by the distance to the listener. Think, for instance, in background music or sound effects related to player controls. If we set these sources as relative, and set their position to  they will not be attenuated.
...
![Listener at and up vectors](.gitbook/assets/listener_at_up.png)

The “at” vector basically points where the listener is facing, by default its coordinates are . The “up” vector determines which direction is up for the listener and, by default it points to . So the tree components of each of those two vectors are what are set in the `alListenerfv` method call. This method is used to transfer a set of floats \(a variable number of floats\) to a property, in this case, the orientation.
...
A final note. OpenAL also allows you to change the attenuation model by using the alDistanceModel and passing the model you want \('\`\`AL11.AL\_EXPONENT\_DISTANCE`,`AL\_EXPONENT\_DISTANCE\_CLAMP\`\`\`, etc.\). You can play with them and check the results.
